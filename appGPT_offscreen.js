...
class RenderScene {
  ...
  initWorker() {
    ...
    const renderWorker = new Worker(new URL('./workers/renderWorker.js', import.meta.url), { type: 'module' });
  }
  ...
  initOffscreenCanvas() {
    ...
    this.renderer = new THREE.WebGLRenderer({
      ...
      context: this.offscreenCanvas.getContext('webgl2', { antialias: false, stencil: false }),
    ...
    // Переносим отрисовку на основной поток
    this.transferToMainThread();
  }

  transferToMainThread() {
    const imageBitmap = this.offscreenCanvas.transferToImageBitmap();
    const mainContext = this.mainCanvas.getContext('bitmaprenderer');
    mainContext.transferFromImageBitmap(imageBitmap);
}
  ...
  animate = (time) => {
    ...
    // Обновление картинки в основном потоке
    this.transferToMainThread();
